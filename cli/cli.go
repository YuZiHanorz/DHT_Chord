...
func (C *Chord) CreateCommand(input ...string) error {
	...
	for key, val := range m {
		...
		if val < 0 {
			fmt.Println("Boom ! delete error")
		}
		if val > 1 {
			fmt.Println("Boom ! put error")
		}
		if val == 1 {
			C.PutCommand(key, p[key])
		}
